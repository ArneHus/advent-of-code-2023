...
public class Part2 {
        ...
        for (int lineIndex = 0; lineIndex < lines.size(); lineIndex++) {
            ...
            for (int charIndex = 0; charIndex < line.length(); charIndex++) {
                ...
                if (symbols.contains(character)) {
                    int gearRatio = findAdjacentNumbers(lines, lineIndex, charIndex);
                    allNumbers.add(gearRatio);
                }
            }
        }
    ...
    private static int findAdjacentNumbers(List<String> lines, int currentLine, int currentCharacter) {
